...
from .models import TaskStatus, TaskPriority, Task, SubTask
...
class KanbanColumn(BaseModel):
    ...
    max_tasks: Optional[int] = Field(default=None, ge=1, description="Maximum number of tasks allowed in this column")
...
class KanbanBoard(BaseModel):
    ...
    task_filters: Dict[str, Any] = Field(default_factory=dict, description="Active filters for tasks")
...
class TaskCardData(BaseModel):
    ...
    subtask_summary: Dict[str, int] = Field(default_factory=dict, description="Summary of subtask statuses")
    dependency_status: Dict[str, str] = Field(default_factory=dict, description="Status of task dependencies")
    urgency_score: Optional[float] = Field(default=None, description="Calculated urgency score")
...
class TimeSeriesDataPoint(BaseModel):
    ...
    label: Optional[str] = Field(default=None, description="Optional label for this point")
...
class VelocityMetrics(BaseModel):
    ...
    tasks_completed: int = Field(..., ge=0, description="Number of tasks completed in period")
    story_points_completed: Optional[float] = Field(default=None, ge=0, description="Story points completed")
    average_cycle_time_hours: Optional[float] = Field(default=None, ge=0, description="Average time from start to completion")
    throughput_per_day: float = Field(..., ge=0, description="Average tasks completed per day")
    predicted_completion_date: Optional[date] = Field(default=None, description="Predicted project completion date")
...
class BurndownData(BaseModel):
    ...
    daily_progress: List[TimeSeriesDataPoint] = Field(..., description="Daily progress data points")
    ideal_burndown: List[TimeSeriesDataPoint] = Field(..., description="Ideal burndown line")
    actual_burndown: List[TimeSeriesDataPoint] = Field(..., description="Actual progress line")
...
class ProjectInsights(BaseModel):
    ...
    generated_at: datetime = Field(default_factory=datetime.now, description="When insights were generated")
    project_health_score: float = Field(..., ge=0, le=100, description="Overall project health score")
    velocity_trend: str = Field(..., description="Velocity trend analysis (improving/declining/stable)")
    bottlenecks: List[Dict[str, Any]] = Field(default_factory=list, description="Identified bottlenecks")
    recommendations: List[str] = Field(default_factory=list, description="AI recommendations for improvement")
    risk_factors: List[Dict[str, Any]] = Field(default_factory=list, description="Identified risk factors")
    predicted_delays: List[Dict[str, Any]] = Field(default_factory=list, description="Predicted task delays")
...
class TaskAssignment(BaseModel):
    ...
    due_date: Optional[datetime] = Field(default=None, description="Due date for the assignment")
...
class Comment(BaseModel):
    ...
    subtask_id: Optional[int] = Field(default=None, description="ID of the subtask if applicable")
    user_id: str = Field(..., description="ID of the user who made the comment")
    content: str = Field(..., min_length=1, description="Comment content")
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: Optional[datetime] = Field(default=None, description="Last edit timestamp")
...
class ActivityEvent(BaseModel):
    ...
    event_type: str = Field(..., description="Type of event (task_created, status_changed, etc.)")
...
class GitHubIntegration(BaseModel):
    ...
    auto_create_issues: bool = Field(default=False, description="Automatically create GitHub issues for tasks")
    sync_commits: bool = Field(default=True, description="Sync commit messages with task progress")
    webhook_url: Optional[str] = Field(default=None, description="Webhook URL for GitHub events")
...
class SlackIntegration(BaseModel):
    ...
    notify_on_completion: bool = Field(default=True, description="Send notifications when tasks are completed")
    notify_on_assignment: bool = Field(default=True, description="Send notifications when tasks are assigned")
...
class ExportRequest(BaseModel):
    ...
    task_ids: Optional[List[int]] = Field(default=None, description="Specific task IDs to export (all if None)")
    include_subtasks: bool = Field(default=True, description="Include subtasks in export")
    include_comments: bool = Field(default=False, description="Include comments in export")
...
class ImportRequest(BaseModel):
    ...
    data: Union[str, Dict[str, Any], List[Dict[str, Any]]] = Field(..., description="Data to import")
    merge_strategy: str = Field(default="append", description="How to handle existing tasks (append/replace/merge)")
    validate_dependencies: bool = Field(default=True, description="Validate task dependencies during import")
...
class TaskFilter(BaseModel):
    ...
    assignee: Optional[List[str]] = Field(default=None, description="Filter by assigned user")
    ...
    due_after: Optional[datetime] = Field(default=None, description="Filter by due date")
    due_before: Optional[datetime] = Field(default=None, description="Filter by due date")
    tags: Optional[List[str]] = Field(default=None, description="Filter by tags")
    search_text: Optional[str] = Field(default=None, description="Full-text search in title/description")
    has_subtasks: Optional[bool] = Field(default=None, description="Filter tasks with/without subtasks")
    complexity_min: Optional[int] = Field(default=None, ge=1, le=10, description="Minimum complexity score")
    complexity_max: Optional[int] = Field(default=None, ge=1, le=10, description="Maximum complexity score")
