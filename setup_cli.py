...
def check_python_version():
    ...
    if version.major < 3 or (version.major == 3 and version.minor < 9):
        print_status(f"Python {version.major}.{version.minor} detected. Python 3.9+ required.", "error")
        return False
...
def test_cli_functionality():
    ...
    commands_to_test = [
        ...
        (f"{python_executable} {cli_script_name} --json usage-stats", "Test 2: --json stats")
    ]
    ...
    for command, test_name in commands_to_test:
        ...
        try:
            ...
            process = subprocess.run(command.split(), capture_output=True, text=True, env=os.environ.copy(), check=False)
            
            if process.returncode == 0:
                print_status(f"{test_name} ✓", "success")
            else:
                print_status(f"{test_name} failed (return code: {process.returncode}):", "error")
                if process.stdout:
                    print_status(f"Subprocess STDOUT:\n{process.stdout.strip()}", "warning")
                if process.stderr:
                    print_status(f"Subprocess STDERR:\n{process.stderr.strip()}", "error")
