...
try:
    ...
    import traceback
    print(f"Detailed Import Error for MCP tools:\nType: {type(e)}\nError: {e}\nTraceback:\n{traceback.format_exc()}")
    print(f"Warning: MCP tools not available: {e}")
    print("Some functionality will be limited. Install dependencies with: pip install fastmcp rich")
    MCP_AVAILABLE = False
    ...
    class _DummyMCPToolWithFn:
        ...
        def _create_dummy_callable(self, tool_name_str_for_dummy_callable):
            def dummy_callable_fn(*args, **kwargs):
                ...
                project_root_val = kwargs.get('project_root', 'unknown_project_root_in_dummy')
    ...
    _tool_names_for_dummies = [
        ...
        "set_task_status_tool", "add_dependency_tool", "remove_dependency_tool",
        "validate_dependencies_tool", "parse_prd_tool", "init_claude_support_tool",
        "get_cache_metrics_tool", "clear_cache_tool", "check_rate_limits_tool",
        "get_usage_stats_tool", "check_budget_status_tool"
    ]
    ...
    def cmd_next(self, args):
        ...
        try:
            result = get_next_task_tool.fn(
                project_root=self.project_root,
                exclude_dependencies=args.ignore_deps
            )
            ...
            if args.json:
                ...
                if next_task:
                    self.print_success(f"Next task: {next_task['id']} - {next_task['title']}")
        ...
        try:
            ...
            if args.json:
                ...
                if issues:
                    ...
                    for issue in issues:
                        self.print_rich(f"  • {issue.get('description', 'Unknown issue')}", style="red")

                warnings = validation.get('warnings', [])
                if warnings:
                    self.print_rich(f"\nWarnings ({len(warnings)}):", style="yellow")
                    for warning in warnings:
                        self.print_rich(f"  • {warning.get('description', 'Unknown warning')}", style="yellow")

                fixes = validation.get('fixes_applied', [])
                if fixes and args.fix:
                    self.print_rich(f"\nFixes applied ({len(fixes)}):", style="green")
                    for fix in fixes:
                        self.print_rich(f"  • {fix.get('type', 'Unknown fix')}", style="green")
        ...
        try:
            ...
            if RICH_AVAILABLE:
                with Progress(
                    ...
                    task = progress.add_task("Parsing PRD and generating tasks...", total=None)
            ...
            if args.json:
                ...
                self.print_success(f"Successfully generated {generated_count} tasks from PRD")

                if 'ai_metadata' in result:
                    metadata = result['ai_metadata']
                    self.print_rich(f"Research used: {metadata.get('research_used', False)}", style="dim")
                    self.print_rich(f"Model used: {metadata.get('model_used', 'unknown')}", style="dim")
    ...
    def cmd_cache_metrics(self, args):
        ...
        try:
            ...
            if args.json:
                ...
                self.print_rich(f"Hit Rate: {cache_metrics.get('hit_rate', 0):.1f}%", style="green")
                ...
                self.print_rich(f"Total Saved Cost: ${cache_metrics.get('total_saved_cost', 0):.4f}")
    ...
    def cmd_usage_stats(self, args):
        ...
        try:
            ...
            if args.json:
                ...
                self.print_rich(f"Cache Hit Rate: {stats.get('cache_hit_rate', 0):.1f}%")
...
Examples:
  ...
  pytaskai config set ai_service.use_research true
    ...
    list_parser.add_argument('--status', choices=['pending', 'in-progress', 'done', 'cancelled', 'review', 'deferred'],
    ...
    list_parser.add_argument('--no-subtasks', action='store_true',
                            help='Exclude subtasks from output')
    ...
    next_parser.add_argument('--ignore-deps', action='store_true',
    ...
    status_parser.add_argument('status', choices=['pending', 'in-progress', 'done', 'cancelled', 'review', 'deferred'],
                              help='New status')
    status_parser.add_argument('--subtask-id', type=int,
                              help='Subtask ID (if updating a subtask)')
    status_parser.add_argument('--no-timestamp', action='store_true',
                              help='Do not update timestamp')
    
    # Dependency management
    add_dep_parser = subparsers.add_parser('add-dep', help='Add task dependency')
    add_dep_parser.add_argument('task_id', type=int, help='Task ID that depends on another')
    add_dep_parser.add_argument('dependency_id', type=int, help='Task ID to depend on')
    add_dep_parser.add_argument('--no-circular-check', action='store_true',
                               help='Skip circular dependency validation')
    
    remove_dep_parser = subparsers.add_parser('remove-dep', help='Remove task dependency')
    remove_dep_parser.add_argument('task_id', type=int, help='Task ID')
    remove_dep_parser.add_argument('dependency_id', type=int, help='Dependency ID to remove')
    
    validate_deps_parser = subparsers.add_parser('validate-deps', help='Validate task dependencies')
    validate_deps_parser.add_argument('--fix', action='store_true',
    ...
    prd_parser.add_argument('--count', type=int, default=10,
                           help='Number of tasks to generate (default: 10)')
    prd_parser.add_argument('--research', action='store_true',
                           help='Use research capabilities')
    prd_parser.add_argument('--latest', action='store_true',
                           help='Prefer latest versions over LTS')
    prd_parser.add_argument('--overwrite', action='store_true',
                           help='Overwrite existing tasks')
    
    # Cache and metrics
    cache_parser = subparsers.add_parser('cache-metrics', help='Show cache performance metrics')
    cache_parser.add_argument('--detailed', action='store_true',
    ...
    config_parser.add_argument('action', choices=['show', 'get', 'set', 'reset'],
                              help='Configuration action')
    config_parser.add_argument('key', nargs='?', help='Configuration key (dot notation)')
    ...
    # Claude Code setup
    init_parser = subparsers.add_parser('init-claude', help='Initialize Claude Code support')
    init_parser.add_argument('--no-windsurfrules', action='store_true',
