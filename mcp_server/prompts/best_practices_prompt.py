...
def create_best_practices_prompt(
    ...
    prompt_parts.extend([
        ...
        "",
        experience_guidance.get(team_experience, experience_guidance["intermediate"]),
        ""
    ])
...
def validate_best_practices_response(response_data: Dict[str, Any]) -> tuple[bool, List[str]]:
    """
    ...
    if "best_practices" in response_data:
        if not isinstance(response_data["best_practices"], list):
            ...
            for i, practice in enumerate(response_data["best_practices"]):
                ...
                required_practice_fields = ["category", "practice", "implementation", "priority"]
                for practice_field in required_practice_fields:
                    if practice_field not in practice:
                        errors.append(f"Best practice {i} missing required field: {practice_field}")
...
def get_domain_specific_best_practices_prompt(domain: str, technology_stack: List[str], team_level: str = "intermediate") -> str:
    """
...
def get_compliance_best_practices_prompt(compliance_standards: List[str], industry: str = "general") -> str:
    """
