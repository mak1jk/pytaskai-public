...
def create_add_task_prompt(
    ...
    if existing_tasks_context:
        ...
        for task in existing_tasks_context[:5]:  # Limit to 5 most relevant tasks
            prompt_parts.append(f"- **Task {task.get('id', '?')}:** {task.get('title', 'Unknown')} ({task.get('status', 'unknown')})")
    ...
    if dependencies:
        prompt_parts.append(f"- Remember this task depends on completion of tasks: {dependencies}")
