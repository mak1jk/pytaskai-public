...
def create_expand_task_prompt(
    ...
    if existing_subtasks:
        ...
        for i, subtask in enumerate(existing_subtasks[:10], 1):  # Limit to 10 for context
        ...
        if len(existing_subtasks) > 10:
            prompt_parts.append(f"... and {len(existing_subtasks) - 10} more existing subtasks")
...
def create_simple_expand_task_prompt(task_title: str, task_description: str, num_subtasks: int = 5) -> str:
    """
...
def validate_expand_task_response(response_data: Dict[str, Any]) -> tuple[bool, List[str]]:
    """
        ...
        if not isinstance(subtasks, list):
            ...
            if "total_subtasks" in response_data:
                ...
                if len(subtasks) != expected_count:
                    errors.append(f"Subtasks count ({len(subtasks)}) doesn't match total_subtasks ({expected_count})")
            ...
            for i, subtask in enumerate(subtasks):
                ...
                required_subtask_fields = ["id", "title", "description", "estimated_hours"]
                for subtask_field in required_subtask_fields:
                    if subtask_field not in subtask:
                        errors.append(f"Subtask {i} missing required field: {subtask_field}")
                ...
                if "estimated_hours" in subtask:
                    try:
                        ...
                        if hours <= 0:
                            errors.append(f"Subtask {i} estimated_hours must be positive")
                    except (ValueError, TypeError):
                        errors.append(f"Subtask {i} estimated_hours must be a valid number")
                
                if "complexity_score" in subtask:
                    try:
                        score = int(subtask["complexity_score"])
                        if not (1 <= score <= 10):
                            errors.append(f"Subtask {i} complexity_score must be between 1 and 10")
                    except (ValueError, TypeError):
                        errors.append(f"Subtask {i} complexity_score must be a valid integer")
