...
class AIService:
    ...
    def _estimate_cost(self, prompt_length: int, response_length: int, model_name: str) -> float:
    ...
    def _setup_model_configurations(self) -> Dict[str, ModelConfig]:
        ...
        env_models = {
            "default_generation": os.getenv("PYTASKAI_DEFAULT_MODEL", defaults["default_generation"]),
            "research_generation": os.getenv("PYTASKAI_RESEARCH_MODEL", defaults["research_generation"]),
            "lts_search": os.getenv("PYTASKAI_LTS_MODEL", defaults["lts_search"]),
            "best_practices_search": os.getenv("PYTASKAI_BEST_PRACTICES_MODEL", defaults["best_practices_search"]),
        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to parse JSON from {model_config.name}, returning raw content")
                parsed_content = {"raw_content": content, "parsing_error": True}
            
            # Calculate metrics
            latency = time.time() - start_time
            usage = response.usage
            cost = self._calculate_cost(usage.total_tokens, model_config.cost_per_1k_tokens)
            estimated_cost = cost
            ...
            logger.info(f"AI call successful: {usage.total_tokens} tokens, ${cost:.4f}, {latency:.2f}s")
    ...
    async def _get_lts_versions(self, technologies: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            if cached_result is not None:
                logger.info(f"Using cached LTS versions for {len(technologies)} technologies")
            ...
            if status == RateLimitStatus.RATE_LIMITED:
                ...
                rate_limit_msg = self.cache_manager.get_rate_limit_message(provider, model_name)
                if rate_limit_msg:
                    logger.error(rate_limit_msg)
                logger.warning(f"Rate limited for {provider}, using fallback or cached data")
                return {}
            ...
            cost_estimate = self._estimate_cost(len(user_prompt), len(str(result)), model_name)
            ...
            logger.info(f"Found and cached LTS versions for {len(lts_versions)} technologies")
            return lts_versions
    ...
    async def _get_best_practices(self, topic: str, context: str = "") -> List[str]:
        ...
        try:
            ...
            logger.info(f"Found and cached {len(best_practices)} best practices for {topic}")
    ...
    async def generate_task_with_ai(
        ...
        **kwargs
        ...
        """
        try:
            logger.info(f"Generating task with research={use_research}, lts_deps={use_lts_deps}")
            ...
            if use_research:
                ...
                if research_topic:
                    research_tasks.append(self._get_best_practices(research_topic, project_context or ""))
                ...
                if research_tasks:
                    ...
                    # Process LTS results
                    if len(research_results) > 0 and isinstance(research_results[0], dict):
                    ...
                    if len(research_results) > 1 and isinstance(research_results[1], list):
                    ...
                    research_findings = "\\n".join(findings_parts)
                    logger.info(f"Research completed with {len(findings_parts)} findings")
    ...
    def _extract_topic(self, text: str) -> str:
        ...
        topic_keywords = {
            ...
            "performance": ["performance", "optimization", "scaling", "caching", "speed"],
    ...
    def _generate_cache_key(self, model_name: str, system_prompt: str, user_prompt: str) -> str:
    ...
    def _update_metrics(self, model_name: str, tokens: int, cost: float, latency: float, success: bool):
        ...
        total_latency = self.usage_metrics.average_latency * (self.usage_metrics.total_calls - 1) + latency
        self.usage_metrics.average_latency = total_latency / self.usage_metrics.total_calls
        
        # Update success rate
        successful_calls = self.usage_metrics.success_rate * (self.usage_metrics.total_calls - 1) / 100
        if success:
            successful_calls += 1
        self.usage_metrics.success_rate = (successful_calls / self.usage_metrics.total_calls) * 100
        
        # Update by-model metrics
        if model_name not in self.usage_metrics.by_model:
            self.usage_metrics.by_model[model_name] = {"calls": 0, "tokens": 0, "cost": 0.0}
    ...
    async def _check_rate_limit(self, model_or_provider: str, model_name: str = None) -> bool:
        ...
        if calls_made >= calls_per_minute:
            logger.warning(f"Rate limit exceeded for {provider}: {calls_made}/{calls_per_minute}")
            return False
...
__all__ = ["AIService", "ModelConfig", "ModelProvider", "AIUsageMetrics", "ResearchResults"]