...
class CacheManager:
    ...
    def _load_ttl_config(self) -> Dict[CacheType, int]:
        ...
        return {
            CacheType.LTS_RESEARCH: int(os.getenv("PYTASKAI_LTS_CACHE_TTL", "86400")),  # 24 hours
            CacheType.BEST_PRACTICES: int(os.getenv("PYTASKAI_BP_CACHE_TTL", "43200")),  # 12 hours
            CacheType.TASK_GENERATION: int(os.getenv("PYTASKAI_TASK_CACHE_TTL", "3600")),  # 1 hour
            CacheType.GENERAL: int(os.getenv("PYTASKAI_GENERAL_CACHE_TTL", "7200"))  # 2 hours
        }
    ...
    def _load_rate_limit_config(self) -> Dict[str, Dict[str, int]]:
        ...
        return {
            ...
            "perplexity": {
                ...
                "minute_limit": int(os.getenv("PYTASKAI_PERPLEXITY_MINUTE_LIMIT", "20"))
            },
            "google": {
    ...
    def _start_cleanup_task(self):
        ...
        if self._cleanup_task is None:
            try:
                ...
                logger.info("No event loop for background cleanup, using manual cleanup")
    ...
    def check_rate_limit(self, provider: str, model: str) -> RateLimitStatus:
        ...
        if (rate_info.first_call_time and 
            now.date() > rate_info.first_call_time.date()):
        ...
        if rate_info.last_call_time:
            ...
            if time_since_last < 60 and rate_info.calls_count % rate_info.minute_limit == 0:
    ...
    def record_api_call(self, provider: str, model: str) -> None:
        ...
        logger.debug(f"Recorded API call for {rate_key} (count: {rate_info.calls_count})")
    ...
    async def wait_for_rate_limit(self, provider: str, model: str) -> float:
        ...
        if status == RateLimitStatus.RATE_LIMITED:
            ...
            wait_time = 60
            logger.info(f"Rate limited, waiting {wait_time} seconds for {provider}:{model}")
        ...
        return {
            ...
            "rate_limits": {
                rate_key: {
                    ...
                    "usage_percentage": round((info.calls_count / info.daily_limit) * 100, 2)
                }
    ...
    def get_cache_stats_by_type(self) -> Dict[str, Dict[str, Any]]:
        ...
        for cache_type in CacheType:
            entries = [entry for entry in self.cache.values() if entry.cache_type == cache_type]
