...
class UsageTracker:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.auto_save = bool(os.getenv("PYTASKAI_AUTO_SAVE_USAGE", "true").lower() == "true")
    ...
    def record_usage(
        ...
        logger.debug(f"Recorded usage: {provider}/{model} - {status.value} - {estimated_cost:.4f}")
    ...
    def get_stats(
        ...
        stats.successful_calls = len([r for r in filtered_records if r.status == CallStatus.SUCCESS])
        stats.failed_calls = len([r for r in filtered_records if r.status == CallStatus.FAILED])
        stats.cached_calls = len([r for r in filtered_records if r.cache_hit])
        stats.rate_limited_calls = len([r for r in filtered_records if r.status == CallStatus.RATE_LIMITED])
        ...
        stats.total_completion_tokens = sum(r.completion_tokens for r in filtered_records)
        ...
        if operation_costs:
            stats.most_expensive_operation = max(operation_costs, key=operation_costs.get)
    ...
    def _get_provider_breakdown(self, records: List[UsageRecord]) -> Dict[str, Dict[str, Any]]:
    ...
    def _get_operation_breakdown(self, records: List[UsageRecord]) -> Dict[str, Dict[str, Any]]:
        ...
        for op_type, data in breakdown.items():
            ...
            data["success_rate"] = (successful / len(op_records)) * 100 if op_records else 0
        
        return breakdown
    
    def _get_daily_trends(self, records: List[UsageRecord]) -> Tuple[List[Tuple[str, float]], List[Tuple[str, int]]]:
    ...
    def check_budget_status(self) -> Dict[str, Any]:
        ...
        daily_usage_pct = (daily_stats.total_cost / self.daily_budget) * 100 if self.daily_budget > 0 else 0
        monthly_usage_pct = (monthly_stats.total_cost / self.monthly_budget) * 100 if self.monthly_budget > 0 else 0
        ...
        return {
            ...
            "recommendations": self._get_budget_recommendations(daily_status, monthly_status, daily_stats, monthly_stats)
        }
    
    def _get_budget_recommendations(self, daily_status: str, monthly_status: str, daily_stats: UsageStats, monthly_stats: UsageStats) -> List[str]:
        """Get budget recommendations based on current usage."""
        recommendations = []
        
        if daily_status == "exceeded":
            recommendations.append("🚨 Daily budget exceeded! Consider pausing AI operations or increasing budget.")
        elif daily_status == "warning":
            recommendations.append("⚠️ Approaching daily budget limit. Monitor usage carefully.")
        
        if monthly_status == "exceeded":
            recommendations.append("🚨 Monthly budget exceeded! Review usage patterns and adjust limits.")
        elif monthly_status == "warning":
            recommendations.append("⚠️ Approaching monthly budget limit. Consider optimizing usage.")
        
        # Efficiency recommendations
        if daily_stats.cache_hit_rate < 30:
            recommendations.append("💡 Low cache hit rate. Enable caching to reduce costs.")
        
        if daily_stats.total_cost > 0 and daily_stats.successful_calls / daily_stats.total_calls < 0.8:
            recommendations.append("💡 High failure rate. Check API configuration to avoid wasted calls.")
        
        # Model recommendations
        if monthly_stats.most_expensive_operation:
            recommendations.append(f"💡 Most expensive operation: {monthly_stats.most_expensive_operation}. Consider optimizing this workflow.")
    ...
    def export_to_csv(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            with open(filename, 'w', newline='') as csvfile:
                fieldnames = [
                    'timestamp', 'provider', 'model', 'operation_type', 'operation_context',
                    'tool_name', 'status', 'prompt_tokens', 'completion_tokens', 'total_tokens',
                    'estimated_cost', 'duration_ms', 'cache_hit', 'error_message'
                ]
    ...
    def get_top_models_by_cost(self, limit: int = 5) -> List[Tuple[str, float]]:
        ...
        for record in self.records:
            ...
            model_costs[model_key] = model_costs.get(model_key, 0) + record.estimated_cost
    ...
    def get_efficiency_metrics(self) -> Dict[str, Any]:
        ...
        successful_calls = len([r for r in self.records if r.status == CallStatus.SUCCESS])
        ...
        cost_per_successful_call = total_cost / successful_calls if successful_calls > 0 else 0
    ...
    def _calculate_cache_savings(self) -> float:
        ...
        for record in cache_hits:
            ...
            if record.cost_per_1k_input > 0:
                estimated_cost = (record.prompt_tokens / 1000) * record.cost_per_1k_input
                estimated_cost += (record.completion_tokens / 1000) * record.cost_per_1k_output
