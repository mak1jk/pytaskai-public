...
def get_individual_task_file_path(task_id: int, project_root: str = ".") -> Path:
    ...
    return get_data_directory(project_root) / DEFAULT_TASKS_DIR / f"task_{task_id:03d}.txt"
...
def _rotate_backups(backup_path: Path) -> None:
    ...
    base_path = backup_path.with_suffix(backup_path.suffix.replace(BACKUP_EXTENSION, ""))
...
def find_subtask_by_id(tasks: List[Task], parent_task_id: int, subtask_id: int) -> Optional[Tuple[Task, SubTask]]:
...
"""
        for subtask in task.subtasks:
            status_emoji = "✅" if subtask.status == TaskStatus.DONE else "⏳" if subtask.status == TaskStatus.IN_PROGRESS else "📋"
            content += f"""
...
def validate_tasks_file(project_root: str = ".") -> Dict[str, Any]:
    ...
    if isinstance(data, dict) and 'tasks' in data:
        ...
        report["issues"].append("Invalid file structure: expected tasks array or object with tasks property")
        return report
        ...
        try:
            ...
            for dep_id in task.dependencies:
                if dep_id not in task_ids and dep_id > task.id:
                    report["warnings"].append(f"Task {task.id} depends on future task {dep_id}")
...
def get_tasks_statistics(tasks: List[Task]) -> Dict[str, Any]:
    ...
    # Calculate averages
    complexity_scores = [task.complexity_score for task in tasks if task.complexity_score is not None]
